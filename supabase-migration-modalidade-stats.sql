-- =============================================================================
-- Migração: Estatísticas por modalidade (admin preenche manualmente)
...
-- Inserir linhas iniciais para cada modalidade
...
CREATE POLICY "Public read modalidade_stats"
  ON public.modalidade_stats FOR SELECT TO anon, authenticated
  USING (true);

DROP POLICY IF EXISTS "Admins manage modalidade_stats" ON public.modalidade_stats;
CREATE POLICY "Admins manage modalidade_stats"
  ON public.modalidade_stats FOR ALL TO authenticated
